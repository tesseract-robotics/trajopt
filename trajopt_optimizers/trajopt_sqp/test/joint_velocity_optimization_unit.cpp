...
void runVelocityConstraintOptimizationTest2(const trajopt_sqp::QPProblem::Ptr& qp_problem)
{
  auto qp_solver = std::make_shared<trajopt_sqp::OSQPEigenSolver>();
  trajopt_sqp::TrustRegionSQPSolver solver(qp_solver);
  qp_solver->solver_->settings()->setVerbosity(DEBUG);
  ...
  const Eigen::VectorXd vel_target = Eigen::VectorXd::Zero(7);
  coeffs = Eigen::VectorXd::Constant(1, 1);
  auto vel_constraint = std::make_shared<trajopt_ifopt::JointVelConstraint2>(vel_target, vars, coeffs, "jv");
  qp_problem->addCostSet(vel_constraint, trajopt_sqp::CostPenaltyType::SQUARED);
...
TEST_F(VelocityConstraintOptimization, velocity_constraint_optimization_trajopt_problem2)  // NOLINT
{
  CONSOLE_BRIDGE_logDebug("VelocityConstraintOptimization, velocity_constraint_optimization_trajopt_problem");
  auto qp_problem = std::make_shared<trajopt_sqp::TrajOptQPProblem>();
  runVelocityConstraintOptimizationTest2(qp_problem);
}
