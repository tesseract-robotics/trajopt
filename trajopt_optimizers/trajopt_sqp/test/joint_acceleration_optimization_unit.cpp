...
class AccelerationConstraintOptimization : public testing::TestWithParam<const char*>
{
public:
  void SetUp() override
  {
...
void runAccelerationConstraintOptimizationTest(const trajopt_sqp::QPProblem::Ptr& qp_problem)
{
  ...
  for (Eigen::Index i = 7; i < 14; i++)
    EXPECT_NEAR(x[i], 3.333, 1e-1);
  for (Eigen::Index i = 14; i < 21; i++)
    EXPECT_NEAR(x[i], 6.666, 1e-1);
...
/** @brief Joint position constraints with a squared acceleration cost in between. Optimized using ipopt */
TEST_F(AccelerationConstraintOptimization, acceleration_constraint_optimization_ipopt)  // NOLINT
{
  // 2) Create Problem
  ...
  const Eigen::VectorXd weights = Eigen::VectorXd::Constant(accel_constraint->GetRows(), 0.01);
  auto accel_cost = std::make_shared<trajopt_ifopt::SquaredCost>(accel_constraint, weights);
  nlp_ipopt.AddCostSet(accel_cost);
...
/** @brief Joint position constraints with a squared acceleration cost in between. Optimized using trajopt_sqp */
