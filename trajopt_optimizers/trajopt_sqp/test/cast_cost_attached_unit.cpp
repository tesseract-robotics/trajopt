...
public:
  ...
  void SetUp() override
  {
    ...
    box_attached_joint.parent_link_id = LinkId("boxbot_link");
    box_attached_joint.child_link_id = LinkId("box_attached");
    ...
    box_attached2_joint.parent_link_id = LinkId("no_geom_link");
    box_attached2_joint.child_link_id = LinkId("box_attached2");
...
void runCastAttachedLinkWithGeomTest(const Environment::Ptr& env, bool fixed_size)
{
  ...
  for (std::size_t i = 1; i < vars.size(); ++i)
  {
    auto collision_evaluator =
        std::make_shared<trajopt_ifopt::LVSContinuousCollisionEvaluator>(manip, env, trajopt_collision_config);
