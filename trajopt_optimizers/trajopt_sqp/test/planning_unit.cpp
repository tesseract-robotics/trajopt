...
void runPlanningTest2(const trajopt_sqp::QPProblem::Ptr& qp_problem, const Environment::Ptr& env)
{
  std::unordered_map<std::string, double> ipos;
  ipos["torso_lift_joint"] = 0;
  ipos["r_shoulder_pan_joint"] = -1.832;
  ...
  {
    const Eigen::VectorXd coeffs = Eigen::VectorXd::Constant(1, 1);
    auto cnt = std::make_shared<JointVelConstraint2>(Eigen::VectorXd::Zero(7), vars, coeffs);
    qp_problem->addCostSet(cnt, trajopt_sqp::CostPenaltyType::SQUARED);
  }
  ...
  {  // Fix start position
    const Eigen::VectorXd coeffs = Eigen::VectorXd::Constant(manip->numJoints(), 5);
    auto cnt = std::make_shared<JointPosConstraint2>(trajectory.row(0), vars[0], coeffs);
    qp_problem->addConstraintSet(cnt);
  }

  {  // Fix end position
    const Eigen::VectorXd coeffs = Eigen::VectorXd::Constant(manip->numJoints(), 5);
    auto cnt = std::make_shared<trajopt_ifopt::JointPosConstraint2>(trajectory.row(5), vars[5], coeffs);
    qp_problem->addConstraintSet(cnt);
  }
  ...
  for (std::size_t i = 1; i < (vars.size() - 1); ++i)
  {
    ...
    auto cnt = std::make_shared<trajopt_ifopt::ContinuousCollisionConstraint2>(
        collision_evaluator, position_vars, vars_fixed[0], vars_fixed[1], 5);
...
TEST_F(PlanningTest, arm_around_table_trajopt_problem2)  // NOLINT
{
  CONSOLE_BRIDGE_logDebug("PlanningTest, arm_around_table2");
  auto qp_problem = std::make_shared<trajopt_sqp::TrajOptQPProblem>();
  runPlanningTest2(qp_problem, env);
}
