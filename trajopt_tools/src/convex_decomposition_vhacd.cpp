...
public:
  ...
  bool run(int argc,char** argv)
  {
    ...
    bool res = interfaceVHACD->Compute(&points[0], (unsigned int)points.size() / 3,
        (const uint32_t *)&triangles[0], (unsigned int)triangles.size() / 3, params_.m_paramsVHACD);
    ...
    if (res)
    {
        ...
        else
        {
            ...
            if (foutCH.is_open()) {
                ...
                for (unsigned int p = 0; p < nConvexHulls; ++p) {
                    interfaceVHACD->GetConvexHull(p, ch);
                    saveOBJ(foutCH, ch.m_points, (const int *)ch.m_triangles, ch.m_nPoints, ch.m_nTriangles, mat, logger_, p, vertexOffset);
...
protected:
  ...
  void parseParameters(int argc,char** argv,Parameters & params)
  {
    for (int i = 1; i < argc; ++i)
    {
      ...
      else if (!strcmp(argv[i], "--maxhulls"               )) { if (++i < argc) params.m_paramsVHACD.m_maxConvexHulls          = atoi(argv[i]); }
  ...
  void usage(const Parameters & params)
  {
      ...
      msg << "       --maxhulls                  Control the max number of convex-hulls generated (default=1024)" << endl;
      ...
      msg << "Usage: "<<std::endl;
      msg << "./mesh_convex_decomposition fileName.off resolution maxDepth maxConcavity planeDownsampling convexhullDownsampling alpha beta pca mode maxCHVertices minVolumePerCH maxConvexHulls outFileName.wrl log.txt"<<std::endl<<std::endl;
      msg << "Examples:" << endl;
