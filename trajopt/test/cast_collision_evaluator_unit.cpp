...
class CastCollisionEvaluatorTest : public testing::Test
{
public:
...
// must be typed Time1. CalcCollisions on joint values does not read the variable vectors, so the
// evaluator is constructed with empty ones.
//
...
TEST_F(CastCollisionEvaluatorTest, SubdividedSegmentEndIsTime1)  // NOLINT
{
  ...
  CastCollisionEvaluator evaluator(
  ...
  ContactResultMap dist_results;
  evaluator.CalcCollisions(dof_vals0, dof_vals1, dist_results);

  ASSERT_FALSE(dist_results.empty());
  const CcTypeCounts counts = checkCcTypeInvariants(dist_results);
  EXPECT_GT(counts.time1, 0) << "Nothing was typed Time1, so the arm marking the end of a subdivided segment never "
                                "fired. Types seen: "
                             << counts.str();
}

// A fixed end variable cannot be moved away from a contact, so a contact at the segment's end is
// dropped rather than costed. That filtering keys off the Time1 type, which is what makes the
// typing above load-bearing rather than cosmetic: the same run typed Between survives the filter
// and contributes a term the solver cannot act on.
TEST_F(CastCollisionEvaluatorTest, SubdividedSegmentEndIsFilteredWhenEndFixed)  // NOLINT
{
  auto manip = env->getJointGroup("manipulator");
  ...
  CastCollisionEvaluator evaluator(
      manip, env, config, sco::VarVector{}, sco::VarVector{}, CollisionExpressionEvaluatorType::START_FREE_END_FIXED);
  ...
  ASSERT_FALSE(dist_results.empty());
  const CcTypeCounts counts = checkCcTypeInvariants(dist_results);
  EXPECT_EQ(counts.time1, 0) << "A contact at the end of the segment survived the fixed-end filter. Types seen: "
                             << counts.str();
}
