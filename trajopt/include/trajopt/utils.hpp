...
struct SafetyMarginData
{
  ...
  SafetyMarginData(const double& default_safety_margin, const double& default_safety_margin_coeff) : default_safety_margin_data_(default_safety_margin, default_safety_margin_coeff), max_safety_margin_(default_safety_margin) {}
   ...
   * @param safety_margin_coeffs A safety margin coefficient vector where each element corresponds to a given timestep.
   */
  void SetPairSafetyMarginData(const std::string& obj1, const std::string& obj2, const double& safety_margin, const double& safety_margin_coeff)
  {
...
private:
  ...
  /// This use when requesting collision data because you can only provide a single contact distance threshold.
