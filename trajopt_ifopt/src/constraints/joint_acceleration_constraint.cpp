...
JointAccelConstraint2::JointAccelConstraint2(const Eigen::VectorXd& targets,
...
Eigen::VectorXd JointAccelConstraint2::GetValues() const
{
  Eigen::VectorXd acceleration(static_cast<std::size_t>(n_dof_) * position_vars_.size());
...
// Set the limits on the constraint values (in this case just the targets)
std::vector<ifopt::Bounds> JointAccelConstraint2::GetBounds() const { return bounds_; }

void JointAccelConstraint2::FillJacobianBlock(std::string var_set, Jacobian& jac_block) const
{
  // Check if this constraint use the var_set
  // Only modify the jacobian if this constraint uses var_set
  if (var_set != var_set_)
