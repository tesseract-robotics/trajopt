...
JointVelConstraint2::JointVelConstraint2(const Eigen::VectorXd& targets,
...
Eigen::VectorXd JointVelConstraint2::GetValues() const
{
  // i - represents the trajectory timestep index
  // k - represents the DOF index
  // var[i, k] - represents the variable index
...
// Set the limits on the constraint values (in this case just the targets)
std::vector<ifopt::Bounds> JointVelConstraint2::GetBounds() const { return bounds_; }

void JointVelConstraint2::FillJacobianBlock(std::string var_set, Jacobian& jac_block) const
{
  // Check if this constraint use the var_set
  // Only modify the jacobian if this constraint uses var_set
