...
JointJerkConstraint2::JointJerkConstraint2(const Eigen::VectorXd& targets,
...
Eigen::VectorXd JointJerkConstraint2::GetValues() const
{
  Eigen::VectorXd acceleration(static_cast<std::size_t>(n_dof_) * position_vars_.size());
  // Forward Diff
  for (std::size_t ind = 0; ind < position_vars_.size() - 3; ind++)
...
// Set the limits on the constraint values (in this case just the targets)
std::vector<ifopt::Bounds> JointJerkConstraint2::GetBounds() const { return bounds_; }

void JointJerkConstraint2::FillJacobianBlock(std::string var_set, Jacobian& jac_block) const
{
  // Check if this constraint use the var_set
  // Only modify the jacobian if this constraint uses var_set
  if (var_set != var_set_)
