...
DiscreteCollisionConstraint2::DiscreteCollisionConstraint2(
    std::shared_ptr<DiscreteCollisionEvaluator> collision_evaluator,
...
// Set the limits on the constraint values
std::vector<ifopt::Bounds> DiscreteCollisionConstraint2::GetBounds() const { return bounds_; }
...
Eigen::VectorXd DiscreteCollisionConstraint2::CalcValues(const Eigen::Ref<const Eigen::VectorXd>& joint_vals) const
{
  // Check the collisions
  const trajopt_common::CollisionCacheData::ConstPtr collision_data =
      collision_evaluator_->CalcCollisions(joint_vals, bounds_.size());
...
void DiscreteCollisionConstraint2::SetBounds(const std::vector<ifopt::Bounds>& bounds)
{
  assert(bounds.size() == 1);
  bounds_ = bounds;
}
...
std::shared_ptr<DiscreteCollisionEvaluator> DiscreteCollisionConstraint2::GetCollisionEvaluator() const
{
  return collision_evaluator_;
}
