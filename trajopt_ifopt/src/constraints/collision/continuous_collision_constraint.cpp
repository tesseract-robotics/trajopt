...
ContinuousCollisionConstraint2::ContinuousCollisionConstraint2(
    std::shared_ptr<ContinuousCollisionEvaluator> collision_evaluator,
...
Eigen::VectorXd ContinuousCollisionConstraint2::GetValues() const
{
  ...
  // const Eigen::VectorXd joint_vals0 = this->GetVariables()->GetComponent(position_vars_[0]->GetName())->GetValues();
  // const Eigen::VectorXd joint_vals1 = this->GetVariables()->GetComponent(position_vars_[1]->GetName())->GetValues();
  const Eigen::VectorXd joint_vals0 = position_vars_[0]->value();
...
void ContinuousCollisionConstraint2::SetBounds(const std::vector<ifopt::Bounds>& bounds)
{
  assert(bounds.size() == 1);
  bounds_ = bounds;
}

std::shared_ptr<ContinuousCollisionEvaluator> ContinuousCollisionConstraint2::GetCollisionEvaluator() const
{
  return collision_evaluator_;
}
